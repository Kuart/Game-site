				...
				for (let i in this.shots) {
					if (this.enemies.length) {
						if (this.shots[i].target.x + this.shots[i].target.sizeX / 2 < 210) {
							...
							this.shots[i].target.x + this.shots[i].target.sizeX / 2 >
							250
						) {
							this.shots[i].dx +=
								(this.shots[i].x - this.shots[i].target.x) *
						...
						this.shots[i].dy +=
							(this.shots[i].y - this.shots[i].target.y) * this.shots[i].speed;
						this.shots[i].y -= this.shots[i].dy;
						if (
							this.shots[i].y <
							this.shots[i].target.y + this.shots[i].target.sizeY / 2
						) {
	...
	wordsGenerator: function(round) {
		for (let i = 0; i < round.types.length; i++) {
			for (let j = 0; j < round.types[i]; j++) {
				if (i === 0) {
					this.words.push(
						this.wordsType1[this.randoming(0, this.wordsType1.length - 1)]
					);
				} else if (i === 1) {
					this.words.push(
						this.wordsType2[this.randoming(0, this.wordsType2.length - 1)]
					);
				} else if (i === 2) {
					this.words.push(
						this.wordsType3[this.randoming(0, this.wordsType3.length - 1)]
					);
				} else if (i === 3) {
					this.words.push(
						this.wordsType4[this.randoming(0, this.wordsType4.length - 1)]
					);
				} else if (i === 4) {
					this.words.push(
						this.wordsType5[this.randoming(0, this.wordsType5.length - 1)]
					);
				}
			}
		}
